...
void TrafficLight::waitForGreen()
{
    ...
    while (true) {
        // pop_back wakes up when a new element is available in the queue
        TrafficLightPhase traffic_light_phase = _message_queue.receive();
        // If traffic light phase is green then stop the waiting
        if ( traffic_light_phase == TrafficLightPhase::green ) {
            return;
        }
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    while(true) {
        ...
        long lasted_time = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - start_time).count();
        
        if (lasted_time >= cycle_duration) {
            ...
            // Update the start time for the new cycle and new cycle duration
